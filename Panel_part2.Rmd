...
And finally, we allow for individual specific trends. To replicate the analysis of @Ludwig.2018.0, we use work experience (`exp`) and squared work experience as the slope variables.
...
@GoodmanBacon.2021 shows that the two-ways FE is a weighted average of all possible two-group/two-period DD estimators. The weights determine how much each of these single combinations contributes to the two-ways FE are determined by the group size (e.g. how long do we observe each combination before and after treatment) and the variance in the treatment.
...
@GoodmanBacon.2021 shows that this setting with three treatment groups consists of 4 possible 2 $\times$ 2 Diff-in-Diff settings.
...
Define $\bar{D_{kl}}$ as the variance of $D_{it}$ in the subsample that compares groups $k$ and $l$, and the relative size of each group in the pair $n_{kl} = \frac{n_k}{n_k + n_l}$. Then, the weights are given by:

$$
W_{jC} = \frac{(n_j + n_C)^2 \hat{V}_D^{jU}}{\hat{V}_D}, ~\text{with}~ \hat{V}_D^{jU} = n_{jU}(1-n_{jU}) \bar{D}_j(1-\bar{D}_j), ~for~ j = k,l. 
$$
...
@GoodmanBacon.2021 also explains the observation of these "negative" weights:
...
Basically, if the treatment effect varies over time (e.g. treatment effect grows over time) TWFE might be biased because early treated groups (with an increasing treatment effect) are used at control groups for late treated groups, thereby masking the treatment effect of these late treated groups (which might however receive a high weight for the overall treatment effect).
...
$$
where $\delta$ is the average tretment effect on the treated (ATT).
...
zp1 <- ggplot(df2, aes(wave, wage)) +
  ...
  geom_point( aes(x = wave, y = wage, shape = Treatment, color = Treatment, alpha = as.factor(D)), 
              size = 2, stroke = 2) +
...
zp2 <- ggplot(df2, aes(wave, wage)) +
  ...
  geom_point( aes(x = wave, y = wage, shape = Treatment, color = Treatment), 
              size = 2, stroke = 2,
              data = df2[df2$D == 2 & df2$wave <= 7, ]) +
  theme_bw() +
  scale_x_continuous( breaks = seq(1, 12, 2))  +
...
```{r, class.source = 'fold-hide', figures-side, fig.show="hold", out.width="50%"}
par(mar = c(4, 4, .1, .1))
zp1
zp2
zp3
