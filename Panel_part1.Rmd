...
This is in long format, with nr as id and year as time indicator. Long format is what we usually need when analysing data.
...
head(Males[, c("nr", "year", "wage", "lag_wage", "lead_wage", "fd_wage")], n = 16)
```
...
One thing that is often difficult: Many panel data come as a bunch of single (cross-sectional) files and have to be combined into a single dataset. However, many of those providers now offer simplified long-format data or pre-written scripts to merge data across waves (e.g. [here](https://www.understandingsociety.ac.uk/documentation/mainstage/syntax) for Understanding Society).
...
Let's clarify the idea and advantage of panel data, lets start with an example. Assume we want to know the relationship between age (as our independent variable) and happiness (as our dependent variable), and we have data on 24 observations.
...
Obviously, we might suspect some other characteristics to influence our results. For instance, birth cohort might be factor that affects age and happiness. We can easily add a control for the cohort so account for this possibility. We would then estiamte the model  

$$
y_{i} = \alpha + \beta_1 x_{i} + \beta_2 z_{it} + \upsilon_{i},
$$
where $z_i$ is the control variable (cohort).
...
We would estimate a marriage premium of more than 2,000. The true value is 500. The reason: those who marry at some point already had higher wages before they married. 
...
Idiosyncratic time-variation in $\epsilon_i$ must be uncorrelated with variation in $x_i$ across all time periods. However, $\E(\alpha_{i} | x_{i})$ can be any function of $x_i$.
...
The RE model is usually written as:
...
This indicates that there is barely any relation between age and happiness. This is because the within and the between estimates cancel our each other. 
